...
/*   Updated: 2018/09/24 15:43:38 by zbatik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cpy_straight_to_mem(t_core *core, t_process *cursor, t_byte *info, int ind)
{
	int i;
	int colour;
	...
	while (++i < REG_SIZE)
	{
		core->mem[(ind + i) % MEM_SIZE] = info[i];
		core->colouring[(ind + i) % MEM_SIZE] = colour;
	}
	return (1);
}
