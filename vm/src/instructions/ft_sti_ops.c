...
/*   Updated: 2018/09/24 14:38:42 by zbatik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int sti_rrr(t_core *core, t_process *cursor, int *ind1, int *ind2)
{
	...
	if (0 ==  convert_reg_to_int(cursor, reg2, ind2))
		return (0);
//	if (PBP)
//		ft_printf(1, na, "RRR endcoding %x: r%d holding and %d and r%d holding %d\n",
...
int sti_rdr(t_core *core, t_process *cursor, int *ind1, int *ind2)
{
	...
	*ind1 = get_dir(core, cursor, 2, e_sti);	
	if (0 == convert_reg_to_int(cursor, reg, ind2))
		return (0);
//	if (PBP)
//		ft_printf(1, na, "RDR endcoding %x: dir %d and r%d holding %d\n",
//				PARA_ENCODE_BYTE, *ind1, reg, *ind2);
	return (1);
}
...
int sti_rir(t_core *core, t_process *cursor, int *ind1, int *ind2)
{
	...
	*ind1 = get_indir(core, cursor, 2, e_sti);
	if (0 == convert_reg_to_int(cursor, reg, ind2))
		return (0);
	//if (PBP)
	//	ft_printf(1, na, "RIR endcoding %x: indir %d reads %d and r%d holding %d\n",
	//			PARA_ENCODE_BYTE, read_from, *ind1, reg, *ind2);
	return (1);
}